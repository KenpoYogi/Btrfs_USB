...
The WSL kernel from Microsoft has no drivers for these, so Linux USB Mounter compiles them with
...
2. In Linux USB Mounter pick **Debian** in the **WSL2 distro** box, then choose **Tools > Build
   filesystem drivers**.
